...
<section class="section-box mt-75">
  ...
  <section class="section-box-2">
    <div class="container">
      <div class="banner-hero banner-image-single"><img style="height: 374px; object-fit: cover;" src="{{ asset($companies->banner) }}" alt="joblist"></div>
      <div class="box-company-profile">
        <div class="row mt-10">
          <div class="col-lg-8 col-md-12">
            <div>
                <img style="width: 100px;height: 100px;object-fit: cover;" src="{{ asset($companies->logo) }}" alt="">
            </div>
            <h5 class="f-18">{{ $companies->name }} <span class="card-location font-regular ml-20">{{ $companies->companyCountry->name }}</span></h5>
          ...
          <div class="sidebar-border">
            <div class="sidebar-heading">
              <div class="avatar-sidebar">
                <div class="sidebar-info pl-0"><span class="sidebar-company">{{ $companies->name }}</span><span
                    class="card-location">{{ $companies->companyCountry->name }}</span></div>
              </div>
            </div>
              ...
              <ul>
                <li>
                  ...
                  <div class="sidebar-text-info"><span class="text-description">Industry Type</span><strong
                      class="small-heading">{{ $companies->industryType?->name }}</strong></div>
                </li>
                <li>
                  <div class="sidebar-icon-item"><i class="fi-rr-marker"></i></div>
                  <div class="sidebar-text-info"><span class="text-description">Organization Type </span><strong
                      class="small-heading">{{ $companies->organizationType?->name }}</strong></div>
                </li>
                <li>
                  ...
                  <div class="sidebar-text-info"><span class="text-description">Team Size</span><strong
                      class="small-heading">{{ $companies->teamSize?->name }}</strong></div>
                </li>
                <li>
                  <div class="sidebar-icon-item"><i class="fi-rr-clock"></i></div>
                  <div class="sidebar-text-info"><span class="text-description">Establis Date</span><strong
                      class="small-heading">{{ formatDate($companies?->establishment_date) }}</strong></div>
                </li>
            ...
            <div class="sidebar-list-job">
              <ul class="ul-disc">
                <li>{{ $companies->address }} {{ $companies->companyCity?->name ? ', '.$companies->companyCity?->name : '' }} {{ $companies->companyState?->name ? ', '.$companies->companyState?->name : ''}} {{ $companies->companyCountry?->name ? ', '.$companies->companyCountry?->name : ''}}</li>
                <li>Phone: {{ $companies->phone }}</li>
                <li>Email: {{ $companies->email }}</li>
                <li>Website: <a href="{{ $companies->website }}">{{ $companies->website }}</a></li>

              </ul>
              <div class="mt-30"><a class="btn btn-send-message"  href="mailto:{{ $companies->email }}">Send Message</a></div>
            </div>
          </div>
        </div>
      </div>
