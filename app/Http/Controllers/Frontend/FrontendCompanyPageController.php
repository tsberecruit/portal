...
class FrontendCompanyPageController extends Controller
{
    function index(Request $request) : View {
        ...
        return view('frontend.pages.company-index', compact('companies', 'countries', 'selectedStates', 'selectedCities', 'industryTypes', 'organizations'));
    }

    function show(string $slug) : View {
        $companies = Company::where(['profile_completion' => 1, 'visibility' => 1, 'slug' => $slug])->firstOrFail();
        $openJobs = Job::where('company_id', $companies->id)->where('status', 'active')->where('deadline', '>=', date('Y-m-d'))->paginate(10);
        return view('frontend.pages.company-details', compact('companies', 'openJobs'));
    }
}
